...
parser.add_argument('--train', type=str, help='path to training data or training csv file (default: none)')
parser.add_argument('--val', type=str, metavar='train_dir', help='path to validation data or csv file')
...
best_acc1 = 0
dist_measure = euclidean_dist  # euclidean_dist


def main():
    args = parser.parse_args()
    global results_dir
    results_dir = os.path.join('/media/D_4TB/YL_4TB/BaoDetection/data/Chanel/ClassificationFilter/sign', args.model_name)

    results_dir = os.path.join(os.path.dirname(args.train), args.model_name)
    mkdir(results_dir)
    global ckpt_dir
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if os.path.isdir(args.val):
        validation_directory = args.val
        val_dataset = ImageFolderFewShot(validation_directory, transforms.Compose([
            transforms.Resize((args.image_size, args.image_size)),
            transforms.ToTensor(),
            normalize,
        ]))
    elif args.val.endswith('csv'):
        val_dataset = MyDatasets(args.val, args.image_size, transforms.Compose([
                transforms.Resize((args.image_size, args.image_size)),
                transforms.ToTensor(),
                normalize,
            ]))
...
def train(train_loader, model, optimizer, epoch, args):
    ...
    for n_episode, batch in enumerate(train_loader, 1):
        ...
        # class_prototypes.shape n_way*512  (512 is the out dim)
